...
6. 对秋招邮件执行动作：
   - 匹配到现有投递（按企业名，同企业多行时结合岗位/部门关键词消歧，仍无法确定则询问用户）→ 更新 `status`：
     ...
     - offer/录用/入职通知 → Offer
     - 感谢信/未通过/不合适 → 已挂
     - 同时 `notes` 追加 `MM-DD 依据（邮件）`，`updatedAt` 更新为当前时间；若邮件给出了具体笔试/面试日期，把 `latestDate` 更新为该日期
   - 未匹配到投递且邮件明确是新投递确认 → 新增一行：id 按 `jm_时间戳_4位hex` 规则生成，`industry` 按预设归类（银行/制造/车企/互联网/央企/证券，无法判断必须询问用户），`status=简历筛`，`source=邮箱提取`
   - 无法确定 → 跳过，在汇总中列为待用户确认
7. 回写两个文件：
